...
class Semver: Codable {
  ...
  convenience init(version: String!) {
    ...
    if valid() {
      let v = versionStr.components(separatedBy: VERSION_DELIMITER) as Array
      major = v[0]
      minor = v[1]
      patch = v[2]

      let prerelease = versionStr.components(separatedBy: PRERELEASE_DELIMITER) as Array
      if prerelease.count > 1 {
        pre = prerelease[1]
      }

      let buildVersion = versionStr.components(separatedBy: BUILD_DELIMITER) as Array
